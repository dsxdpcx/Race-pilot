<template>
  <div>
    ...
    <el-card>
      ...
      <!--参数运动员列表 stripe隔行变色-->
      <el-table :data="caipanList" border stripe>
        <!--索引列-->
        ...
        <el-table-column label="操作" prop="state">
          <template slot-scope="scope">
            ...
            <el-button
                ...
                size="mini"
                type="danger"
                @click="deleteCaipan(scope.row.caipanId)"
            ></el-button>
          </template>
        </el-table-column>
...
//获取运动会届时
    ...
    async exportExcel() {
      ...
      axios
          .get("/excel/exportItemCaipan?item.season.seasonId=" + _this.selectSeasonId + "&item.parentId=" + _this.selectItemId, {
    ...
    async deleteCaipan(caipanId) {
      ...
      axios
          .delete("/caipan/deleteCaipan?caipanId=" + caipanId)
          .then((res) => {
            if (res.data.status == 200) {
